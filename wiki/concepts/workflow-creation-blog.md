...
status: active
updated: 2026-07-07
version: 17 — Ajout maillage : interdiction /collections/all → collection SEO dédiée, lien fiche produit best-seller (jus vers money pages), irrigation du pilier, cible = sujet de l'article
---
...
|---|---|
| Intro/transition (avant sommaire) | 1 phrase suffit si le TL;DR est complet |
...
| 3 | TL;DR — titre spécifique (H2) + texte libre | Juste après le bloc IA — tous les articles |
| 4 | À retenir | Juste après le TLDR — tous les articles |
| 5 | Intro PAS | 100-150 mots |
| 6 | Sommaire | Liens ancres vers les H2 |
| 7 | Section méthodologie | Après sommaire — comparatifs et listes uniquement |
| 8 | Corps de l'article | H2 → H3 |
| 9 | Section "Pour qui ?" | Dans le corps ou en fin — tous les articles |
| 10 | Encadré "Notre recommandation" | Avant la FAQ — comparatifs, listes, avis |
| 11 | Section "Ressources" | Toute fin — tous les articles |
...
#### TL;DR — H2 + texte libre (pas de cadre)

```html
<h2>[emoji] TL;DR – [Titre générique "Réponse rapide" OU sujet spécifique]</h2>
...
Règles :
- **Titre H2** : deux options valides — "🤖 TL;DR – Réponse rapide" (générique, 3/4 articles chatseo) OU "[emoji] TL;DR – [sujet spécifique + année]" (1/4 articles). Les deux sont corrects.
- **Emoji avant TL;DR** : toujours présent (🎯 🤖 📊 selon le sujet)
- **Première phrase en gras** = réponse directe à la question du titre — candidat au featured snippet Google
- **Pas de cadre** — texte libre, pas de `.blog-tldr-box`
- **"La réponse courte :"** suivi de bullets typés (catégorie en gras + réponse)
- L'intro PAS (paragraphes après À retenir) se réduit à **1 phrase de transition** vers le sommaire, car le TL;DR a déjà tout dit

#### Box "À retenir" — HTML à insérer juste après le TLDR
...
3 points max, 1 ligne chacun. Différent du TLDR : le TLDR répond à la question du titre, "À retenir" donne les faits bruts à mémoriser (chiffres, règles, distinctions clés).
...
- [ ] **Box TLDR** après le 1er paragraphe — 3-5 bullet points, réponse directe
- [ ] **Box "À retenir"** après le TLDR — 3 faits clés, 1 ligne chacun
